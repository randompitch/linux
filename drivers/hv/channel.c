...
void vmbus_spin_lock_unlock_irqsave_add_tail(unsigned long flags, struct list_head *entry) {
	spin_lock_irqsave(&vmbus_connection.channelmsg_lock, flags);
        list_add_tail(entry, &vmbus_connection.chn_msg_list);
        spin_unlock_irqrestore(&vmbus_connection.channelmsg_lock, flags);
}
EXPORT_SYMBOL_GPL(vmbus_spin_lock_unlock_irqsave_add_tail);
...
EXPORT_SYMBOL_GPL(vmbus_spin_lock_unlock_irqsave_del);
